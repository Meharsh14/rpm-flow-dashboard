...
URL_VOLUME = f"{BASE_URL}/TotalVolume.json"  # optional (ESP32 side)
...
if "history" not in st.session_state:
    st.session_state.history = pd.DataFrame(
        columns=["Timestamp", "Elapsed_Time_sec", "RPM", "FlowRate", "TotalVolume"]
    )
...
try:
    ...
    if rpm_resp.status_code == 200 and flow_resp.status_code == 200:

        rpm = safe_float(rpm_resp.json())
        flow = safe_float(flow_resp.json())  # L/min

        now = time.time()
        delta_sec = now - st.session_state.prev_time
        st.session_state.prev_time = now

        # ---------------- TIMER & VOLUME ----------------
        if st.session_state.running:
            st.session_state.elapsed_sec = int(now - st.session_state.start_time)
            st.session_state.total_volume += (flow / 60.0) * delta_sec
        ...
        with c3:
            st.plotly_chart(
                go.Figure(go.Indicator(
                    mode="gauge+number",
                    value=round(st.session_state.total_volume, 2),
...
if stop_clicked and not st.session_state.history.empty:
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        st.session_state.history.to_excel(writer, index=False, sheet_name="CapturedData")
    output.seek(0)
