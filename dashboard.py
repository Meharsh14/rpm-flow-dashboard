...
def update_rpm_flow():
    try:
        data_str = request.form.get("data", "0,0")
...
st.title("⚙️ Real-Time RPM & Flow Dashboard")
...
if not st.session_state.run and not st.session_state.data.empty:
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        st.session_state.data.to_excel(writer, index=False, sheet_name="RPM_Flow")
    st.download_button(
        label="📥 Download Excel",
        data=buffer,
        file_name="rpm_flow.xlsx",
